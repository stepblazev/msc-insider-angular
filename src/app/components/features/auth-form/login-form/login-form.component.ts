...
export class LoginFormComponent implements OnInit{
  ...
  public get loginControlHasError() {
    return this.loginControl.errors && Object.keys(this.loginControl.errors).length
  }
  ...
  public async onSubmit() {
    ...
    this._authRepository.login({
      login: this.form.get('login')?.value?.trim() as string,
      password: this.form.get('password')?.value?.trim() as string,
    }).pipe(
      catchError<ILoginResponseDTO, ObservableInput<ILoginResponseDTO>>((selector) => {
        this.form.enable();
        this.isLoading = false;
        this._notifierService.notify("error", "Не удалось выполнить вход...")
        return selector;
      })
